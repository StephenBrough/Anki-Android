...
object MethodLogger {
    ...
    private fun logInternal(message: String) {
        ...
        while (logInternalIndex < size) {
            if (TextUtils.equals(stack[logInternalIndex].className, MethodLogger::class.java.name) && TextUtils.equals(stack[logInternalIndex].methodName, "logInternal")) {
                break
            }
            ++logInternalIndex
        }
        if (logInternalIndex + 2 >= size) {
            throw IllegalStateException("there should always be a caller for this method")
        }

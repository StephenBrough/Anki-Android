...
public class Connection extends BaseAsyncTask<Connection.Payload, Object, Connection.Payload> {
    ...
    private Payload doInBackgroundLogin(Payload data) {
        ...
        try {
            ...
            data.success = false;
            data.result = new Object[] { "errorSnackbar", e.getResponseCode(), e.getMessage() };
            return data;
        } catch (Exception e2) {
...
//                AnkiDroidApp.sendExceptionReport(e2, "doInBackgroundLogin");
    ...
    private Payload doInBackgroundSync(Payload data) {
        ...
        try {
            ...
            // runImport sync and check state
            ...
            if (conflictResolution == null) {
                ...
                if (!retCode.equals("noChanges") && !retCode.equals("success")) {
                    data.success = false;
                    data.result = ret;
                    // Check if there was a sanity check errorSnackbar
...
//            AnkiDroidApp.sendExceptionReport(e, "doInBackgroundSync");
            ...
            Timber.e("doInBackgroundSync -- unknown response code errorSnackbar");
            e.printStackTrace();
            data.success = false;
            Integer code = e.getResponseCode();
            String msg = e.getLocalizedMessage();
            data.result = new Object[] { "errorSnackbar", code , msg };
            return data;
        } catch (Exception e) {
            // Global errorSnackbar catcher.
            // Try to give a human readable errorSnackbar, otherwise print the raw errorSnackbar message
            Timber.e("doInBackgroundSync errorSnackbar");
            e.printStackTrace();
            data.success = false;
            if (timeoutOccured(e)) {
                data.result = new Object[]{"connectionError"};

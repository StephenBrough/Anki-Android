...
class NumberRangePreference : EditTextPreference {
    ...
    var value: Int
        ...
        set(value) {
            ...
            text = Integer.toString(validated)
    ...
    private fun getValidatedRangeFromString(input: String): Int {
        return if (TextUtils.isEmpty(input)) {
            ...
            try {
                getValidatedRangeFromInt(Integer.parseInt(input))
     ...
     * @return The input value within acceptable range.
     */
    private fun getValidatedRangeFromInt(input: Int): Int {
        var input = input
        if (input < mMin) {
            input = mMin
        } else if (input > mMax) {
    ...
    private fun getMaxFromAttributes(attrs: AttributeSet?): Int {
        return attrs?.getAttributeIntValue(AnkiDroidApp.XML_CUSTOM_NAMESPACE, "max",
    ...
    private fun updateSettings() {
        ...
        newFilters[newFilters.size - 1] = InputFilter.LengthFilter(maxLength)

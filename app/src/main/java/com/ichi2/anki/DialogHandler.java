...
public class DialogHandler extends Handler {
    ...
    public void handleMessage(Message msg) {
        ...
        if (msg.what == MSG_SHOW_COLLECTION_LOADING_ERROR_DIALOG) {
            ...
            ((DeckPickerActivity) mActivity.get()).showDatabaseErrorDialog(DatabaseErrorDialog.DIALOG_LOAD_FAILED);
        } else if (msg.what == MSG_SHOW_COLLECTION_IMPORT_REPLACE_DIALOG) {
            // Handle import of collection package APKG
            ((DeckPickerActivity) mActivity.get()).showImportDialog(ImportDialog.DIALOG_IMPORT_REPLACE_CONFIRM, msgData.getString("importPath"));
        } else if (msg.what == MSG_SHOW_COLLECTION_IMPORT_ADD_DIALOG) {
            // Handle import of deck package APKG
            ((DeckPickerActivity) mActivity.get()).showImportDialog(ImportDialog.DIALOG_IMPORT_ADD_CONFIRM, msgData.getString("importPath"));
            ...
            ((DeckPickerActivity) mActivity.get()).showSyncErrorDialog(id, message);
            ...
            if (id!= MediaCheckDialog.DIALOG_CONFIRM_MEDIA_CHECK) {
                ...
                ((DeckPickerActivity) mActivity.get()).showMediaCheckDialog(id, checkList);
            }
        } else if (msg.what == MSG_SHOW_DATABASE_ERROR_DIALOG) {
            // Database error dialog
            ((DeckPickerActivity) mActivity.get()).showDatabaseErrorDialog(msgData.getInt("dialogType"));
            ...
            if (!limited && hkey.length() > 0 && Connection.isOnline()) {
                ((DeckPickerActivity) mActivity.get()).sync();
            } else {

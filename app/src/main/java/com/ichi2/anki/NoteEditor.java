...
import com.ichi2.anki.flashcardviewer.AbstractFlashcardViewer;
...
import com.ichi2.anki.junkdrawer.servicelayer.NoteService;
...
import com.ichi2.anki.junkdrawer.widgets.PopupMenuWithIcons;
...
//    public static final String TARGET_LANGUAGE = "TARGET_LANGUAGE";
    ...
    protected void onStop() {
        ...
        if (!isFinishing()) {
            ...
            UIUtils.INSTANCE.saveCollectionInBackground();
        }
    }
    ...
    private void saveNote() {
        ...
        if (mAddNote) {
            ...
            DeckTask.launchDeckTask(DeckTask.TASK_TYPE_ADD_FACT, mSaveFactHandler, new TaskData(mEditorNote));
        } else {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_reschedule_card:
                ...
                showDialogFragment(NoteEditorRescheduleCard.Companion.newInstance());
                return true;
    ...
    private void showTagsDialog() {
        ...
        TagsDialog dialog = TagsDialog.Companion.newInstance(TagsDialog.TYPE_ADD_TAG, selTags,
                tags);
    ...
    private void showCardTemplateEditor() {
        ...
        startActivityForResult(intent, REQUEST_TEMPLATE_EDIT);
    }
        ...
        switch (requestCode) {
            ...
            case REQUEST_MULTIMEDIA_EDIT:
                if (resultCode != RESULT_CANCELED) {
                    ...
                    int index = extras.getInt(MultimediaEditFieldActivity.Companion.getEXTRA_RESULT_FIELD_INDEX());
                    IField field = (IField) extras.get(MultimediaEditFieldActivity.Companion.getEXTRA_RESULT_FIELD());
                    IMultimediaEditableNote mNote = NoteService.INSTANCE.createEmptyNote(mEditorNote.model());
                    NoteService.INSTANCE.updateMultimediaNoteFromJsonNote(col, mEditorNote, mNote);
                    ...
                    NoteService.INSTANCE.saveMedia(col, (MultimediaEditableNote) mNote);
                    mChanged = true;
                }
                break;
    ...
    private void startMultimediaFieldEditor(final int index, IMultimediaEditableNote mNote, IField field) {
        ...
        editCard.putExtra(MultimediaEditFieldActivity.Companion.getEXTRA_FIELD_INDEX(), index);
        editCard.putExtra(MultimediaEditFieldActivity.Companion.getEXTRA_FIELD(), field);
        editCard.putExtra(MultimediaEditFieldActivity.Companion.getEXTRA_WHOLE_NOTE(), mNote);
    ...
    private boolean duplicateCheck() {
        ...
        // Update the field in the Note so we can runImport a dupe check on it.
        updateField(field);
    ...
    private void updateDeckPosition() {
        ...
        if (position != -1) {
            ...
            Timber.e("updateDeckPosition() errorSnackbar :: mCurrentDid=%d, position=%d", mCurrentDid, position);
        }
    }
    ...
    private boolean updateField(FieldEditText field) {
        String newValue = field.getText().toString().replace(FieldEditText.Companion.getNEW_LINE(), "<br>");

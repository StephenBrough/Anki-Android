...
object BeolingusParser {
    private val PRONUNC_STOPPER = "<img src=\"/pics/s1.png\""
    private val MP3_STOPPER = ".mp3\">Listen"
     ...
     * link, [](... and find there the address with pronunciation page, which is returned)
     * */
    fun getPronounciationAddressFromTranslation(translationHtml: String, wordToSearchFor: String): String {
        val pronounciationIndicator = PRONUNC_STOPPER
        if (!translationHtml.contains(pronounciationIndicator)) {
            return "no"
        }

        // TODO: no do-while loops
        var indIndicator = 0
        do {
            ...
            val indTitle = translationHtml.indexOf(title, indIndicator)

            if (indTitle == -1) {
                return "no"
            }

            val indNextQuote = translationHtml.indexOf("\"", indTitle + title.length)
            if (indNextQuote == -1) {
                return "no"
            }

            // Must be equal to the word translating
            val titleValue = translationHtml.substring(indTitle + title.length, indNextQuote)
        ...
        val indNextQuote = translationHtml.indexOf("\"", indIndicator + href.length)
        if (indNextQuote == -1) {
            return "no"
        }

        val pronounciationAddress = translationHtml.substring(indIndicator + href.length, indNextQuote)
    ...
    fun getMp3AddressFromPronounciation(pronunciationPageHtml: String): String {
        ...
        var indMp3 = pronunciationPageHtml.indexOf(mp3)
        val indAddrEnd = indMp3 + ".mp3".length

        var addrStart = 0
        // Back to find the address start;
        while (indMp3 > 0) {
            indMp3 -= 1
            if (pronunciationPageHtml[indMp3] == '\"') {
                addrStart = indMp3 + 1
                break
            }

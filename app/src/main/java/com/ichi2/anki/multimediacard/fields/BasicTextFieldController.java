...
import com.ichi2.anki.multimediacard.dialogs.PickStringDialogFragment;
...
public class BasicTextFieldController extends FieldControllerBase implements IFieldController,
    ...
    public void createUI(Context context, LinearLayout layout) {
        mEditText = new EditText(getMActivity());
        mEditText.setMinLines(3);
        mEditText.setText(getMField().getText());
        layout.addView(mEditText, LinearLayout.LayoutParams.MATCH_PARENT);

        LinearLayout layoutTools = new LinearLayout(getMActivity());
    ...
    private String gtxt(int id) {
        return getMActivity().getText(id).toString();
    }



    private void createClearButton(LinearLayout layoutTools, LayoutParams p) {
        Button clearButton = new Button(getMActivity());
    ...
    private void createPronounceButton(LinearLayout layoutTools, LayoutParams p) {
        Button btnPronounce = new Button(getMActivity());
        ...
        btnPronounce.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent intent = new Intent(getMActivity(), LoadPronounciationActivity.class);
                intent.putExtra(LoadPronounciationActivity.Companion.getEXTRA_SOURCE(), source);
                getMActivity().startActivityForResult(intent, REQUEST_CODE_PRONOUNCIATION);
            }
        });
    ...
    private void createTranslateButton(LinearLayout layoutTool, LayoutParams ps) {
        Button btnTranslate = new Button(getMActivity());
        ...
        btnTranslate.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!CompatHelper.Companion.isChromebook()) {
                    translationSources.add("ColorDict");
                }

                // Pick from two translation sources
                PickStringDialogFragment.Companion.show(getMActivity().getSupportFragmentManager(),
    ...
    private void createCloneButton(LinearLayout layoutTools, LayoutParams p) {
        ...
        if (getMNote().getNumberOfFields() > 1) {
            ...
            int numTextFields = 0;
            for (int i = 0; i < getMNote().getNumberOfFields(); ++i) {
                // Sort out non text and empty fields
                IField curField = getMNote().getField(i);
                if (curField == null) {
                    continue;
                }
                ...
                if (curField.getText().contentEquals(getMField().getText())) {
                    continue;
                }
            ...
            Button btnOtherField = new Button(getMActivity());
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE_TRANSLATE_GLOSBE && resultCode == Activity.RESULT_OK) {
            ...
            try {
                String pronuncPath = data.getExtras().get(LoadPronounciationActivity.Companion.getEXTRA_PRONUNCIATION_FILE_PATH())
                        .toString();
                ...
                getMActivity().handleFieldChanged(af);
            } catch (Exception e) {
                showToast(gtxt(R.string.multimedia_editor_pron_pronunciation_failed));
            }
    ...
    @Override
    public void onDone() {
        getMField().setText(mEditText.getText().toString());
    }
    ...
    private void showToast(CharSequence text) {
        ...
        Toast toast = Toast.makeText(getMActivity(), text, duration);
        toast.show();
    }
    ...
    protected void startTranslationWithColorDict() {
        ...
        if (!isIntentAvailable(getMActivity(), intent)) {
            showToast(gtxt(R.string.multimedia_editor_trans_install_color_dict));
            return;
        }
        getMActivity().startActivityForResult(intent, REQUEST_CODE_TRANSLATE_COLORDICT);
    }
    ...
    protected void startTranslationWithGlosbe() {
        ...
        Intent intent = new Intent(getMActivity(), TranslationActivity.class);
        intent.putExtra(TranslationActivity.EXTRA_SOURCE, source);
        getMActivity().startActivityForResult(intent, REQUEST_CODE_TRANSLATE_GLOSBE);
    }

...
public class DB {
    ...
    public class MyDbErrorHandler implements DatabaseErrorHandler {
        ...
        public void onCorruption(SQLiteDatabase db) {
            ...
            DatabaseErrorDialog.Companion.setDatabaseCorruptFlag(true);
        }
    }
    ...
    public <T> ArrayList<T> queryColumn(Class<T> type, String query, int column) {
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                try {
                    ...
                    if (cursor.isNull(column)) { // null value encountered
                        nullExceptionCount++;
                        if (nullExceptionCount == 1) { // Toast and errorSnackbar report first time only
                            nullException = e;
            ...
            // This is really coding errorSnackbar, so it should be revealed if it ever happens
            throw new RuntimeException(e);
        } finally {
            if (cursor != null) {
                cursor.close();
            ...
            if (nullExceptionCount > 0) {
                if (nullException != null) {
                    ...
                    StringBuilder sb = new StringBuilder();
                    sb.append("DB.queryColumn(): Critical errorSnackbar -- ");
                    sb.append("unable to pass in the actual exception to errorSnackbar reporting.");
